...
export async function POST() {
  try {
    const existingAdmin = await prisma.user.findUnique({ where: { email: "sbananayo98@gmail.com" } });
    if (existingAdmin) {
      return successResponse(null, "Database already seeded");
    }
    ...
    return successResponse(
      {
        admin: { email: admin.email, password: "Test@12345" },
        lecturers: lecturers.map((l) => ({ email: l.email, password: "lecturer123" })),
        students: students.map((s) => ({ email: s.email, password: "student123", regNo: s.registrationNumber })),
        faculties: faculties.map((f) => f.code),
