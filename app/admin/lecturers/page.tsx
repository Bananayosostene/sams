...
export default function LecturersPage() {
  ...
  const toggleCourse = (id: string) => {
    setForm((f) => ({
      ...f,
      assignedCourses: f.assignedCourses.includes(id)
        ? f.assignedCourses.filter((c) => c !== id)
        : [...f.assignedCourses, id],
    }));
  };
  ...
  const confirmDelete = (l: Lecturer) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <Header
        title="Lecturers"
        subtitle="Manage lecturer accounts and course assignments"
      />
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="card">
          <div className="flex items-center justify-between mb-6">
            <p className="text-sm text-gray-500">
              {lecturers.length} lecturers registered
            </p>
            <button
              onClick={() => {
                setForm({
