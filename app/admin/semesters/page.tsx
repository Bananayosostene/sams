...
export default function SemestersPage() {
  ...
  useEffect(() => {
    api.get<Semester[]>("/api/semesters").then((res) => { setSemesters(res.data); setLoading(false); });
  }, []);
  ...
  const handleEdit = (s: Semester) => {
    setForm({ name: s.name, academicYear: s.academicYear, startDate: s.startDate || "", endDate: s.endDate || "" });
    setEditing(s.id);
    setShowModal(true);
  };
  ...
  if (loading) return <div className="flex flex-col h-full"><Header title="Semesters" subtitle="Manage academic semesters" /><main className="flex-1 p-8"><p>Loading...</p></main></div>;
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="card">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => { setForm({ name: "", academicYear: "", startDate: "", endDate: "" }); setEditing(null); setShowModal(true); }} className="btn-primary">+ Add Semester</button>
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {semesters.map((s) => (
                  <tr key={s.id} className="hover:bg-blue-50/40 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-sm text-gray-600">{s.academicYear}</td>
                    <td className="px-4 py-3.5 text-sm text-gray-600">{s.startDate || "—"}</td>
                    <td className="px-4 py-3.5 text-sm text-gray-600">{s.endDate || "—"}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h3 className="text-lg font-bold text-blue-900 mb-5">{editing ? "Edit Semester" : "Add New Semester"}</h3>
            <div className="space-y-4">
              ...
              <div>
                ...
                <input className="input" placeholder="e.g. 2025/2026" value={form.academicYear} onChange={(e) => setForm({ ...form, academicYear: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1.5">Start Month</label>
                  <input className="input" placeholder="e.g. Jan 2025" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} />
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1.5">End Month</label>
                  <input className="input" placeholder="e.g. Jun 2025" value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} />
                </div>
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={handleAdd} className="btn-primary flex-1">{editing ? "Update Semester" : "Add Semester"}</button>
